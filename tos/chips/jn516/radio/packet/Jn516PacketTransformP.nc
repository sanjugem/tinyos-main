...
implementation {
	...
	command error_t Jn516PacketTransform.TosToMMAC(message_t* from, tsMacFrame* to) {
...
//#endif
	...
	command error_t Jn516PacketTransform.MMACToTos(tsMacFrame* from, message_t* to) {
    	jn516_header_t *to_hdr = call Jn516PacketBody.getHeader(to);
		void* to_payload = (void*)(call Jn516PacketBody.getPayload(to));
		void* from_payload;
		...
		to_hdr->length = sizeof(jn516_header_t) + from->u8PayloadLength - 1;
